...

{
    ...
    my $annotated_bed_File = "$bed.Annotated";
    ...
    my $cmd="\$1\"\\t\"\$2\"\\t\"\$3\"\\t\"\$4\"\\t\"\$11\"\\t\"\$19\"\\t\"\$27\"\\t\"\$6\"\\t\"\$13\"\\t\"\$21\"\\t\"\$29\"\\t\"\$5\"\\t\"\$12\"\\t\"\$20\"\\t\"\$28";
...
sub thread {
    ...
    while (my $bam_file = $q->dequeue_nb()){
        ...
        if ($wholeG) {
            excid_main_WG("$out_name.wholeGenomeCov.fasta",$min,$bed);
        }else{
            excid_main("$out_name.cov.fasta",$min,$bed);
        }
